    ...
    <div class="modal-box" style="max-width:640px;" role="dialog" aria-modal="true" aria-labelledby="product-title">
      ...
      <div style="display:flex;gap:20px;flex-wrap:wrap;margin-bottom:20px;">
        ...
        <div style="flex:1;min-width:200px;">
          ...
          <div style="margin-bottom:14px;">
            ${product.originalPrice ? `<span style="font-size:13px;color:var(--muted);text-decoration:line-through;margin-right:8px;">Bs${product.originalPrice.toFixed(2)}</span>` : ''}
            <span style="font-size:22px;font-weight:800;color:#a4d96f;">Bs4${product.price.toFixed(2)}</span>
          </div>
  ...
  <div>
    ...
    <div style="max-width:var(--maxw);margin:0 auto;padding:0 20px;">
      ...
      <div class="catalog-tools" aria-label="Filtros avanzados">
        ...
        <select id="catalog-price"><option value="all">Todos los precios</option><option value="under25" ${catalogFilters.price === 'under25' ? 'selected' : ''}>Menos de Bs25</option><option value="25to50" ${catalogFilters.price === '25to50' ? 'selected' : ''}>Bs25 a Bs50</option><option value="over50" ${catalogFilters.price === 'over50' ? 'selected' : ''}>Más de Bs50</option></select>
  ...
  <article class="game-card" data-view-product="game-${g.id}" style="cursor:pointer;background:var(--surface);border:1px solid rgba(255,255,255,0.05);border-radius:12px;overflow:hidden;box-shadow:0 6px 20px rgba(0,0,0,0.6);display:flex;flex-direction:column;">
    ...
    <div style="padding:14px 14px 16px;display:flex;flex-direction:column;gap:8px;flex:1;">
      ...
      <div style="margin-top:auto;display:flex;align-items:center;justify-content:space-between;gap:10px;">
        <div>
          ${g.originalPrice ? `<span style="font-size:11.5px;color:var(--muted);text-decoration:line-through;margin-right:6px;">Bs${g.originalPrice.toFixed(2)}</span>` : ''}
          <span style="font-size:16px;font-weight:800;color:#a4d96f;">Bs${g.price.toFixed(2)}</span>
        </div>
        ...
        <article class="game-card" data-view-product="consola-${c.id}" style="cursor:pointer;background:var(--surface);border:1px solid rgba(255,255,255,0.05);border-radius:12px;overflow:hidden;box-shadow:0 6px 20px rgba(0,0,0,0.6);display:flex;flex-direction:column;">
          ...
          <div style="padding:14px 14px 16px;display:flex;flex-direction:column;gap:8px;">
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;margin-top:4px;">
              <span style="font-size:17px;font-weight:800;color:#a4d96f;">Bs${c.price.toFixed(2)}</span>
  ...
  ">
    ...
    ">
      ...
      <p style="
        ...
        color:var(--muted);
      ">
        Administración de las cuentas de PixelStore.
      </p>
    ...
    ">
      ...
      ${users.map(u => `
        ...
        <article style="
          background:var(--surface2,rgba(255,255,255,0.025));
          border:1px solid var(--border-subtle,rgba(255,255,255,0.07));
          border-radius:14px;
          padding:20px;
          transition:transform 160ms,border-color 160ms;
          ${u.banned ? 'opacity:0.6;' : ''}
        "

        onmouseenter="
          this.style.transform='translateY(-3px)';
          this.style.borderColor='rgba(0,229,255,0.25)';
        "

        onmouseleave="
          this.style.transform='none';
          this.style.borderColor='var(--border-subtle,rgba(255,255,255,0.07))';
        "
            ...
            <tbody>
              ${vendorProducts.length === 0 ? `<tr><td colspan="5" style="padding:24px 16px;text-align:center;font-size:13px;color:var(--muted);">No hay productos en el catálogo.</td></tr>` : vendorProducts.map((p, i) => `
                <tr style="border-bottom:${i < vendorProducts.length-1 ? '1px solid rgba(255,255,255,0.03)' : 'none'};transition:background 160ms;"
                  ...
                  <td style="padding:13px 16px;font-size:13.5px;font-weight:700;color:#a4d96f;white-space:nowrap;">Bs${p.price.toFixed(2)}</td>
...
function renderVendor({ currentUser, vendorProducts, vendorSales, orders = [] }) {
  ...
  const liveSales = orders.flatMap(order =>
    order.items
      .filter(item => item.vendorId === currentUser?.id)
      .map(item => ({
        id: order.id,
        game: item.title,
        buyer: order.buyerId,
  ...
  // Combinar ventas reales y ventas del vendedor
  const sales = [...liveSales, ...vendorSales].map((sale, index) => {

    // Buscar el producto correspondiente para obtener su imagen
    ...
    return {
      ...sale,
      image: sale.image || product?.image || '',
      date: sale.date || [
        '15/01/2026',
        '03/03/2026',
        '21/05/2026',
      ...
      ">
        ...
        <div>
          ...
          <p style="
            ...
            font-size:12px;
          ">
            Videojuegos que tienes actualmente publicados.
          </p>
              ...
              <div style="padding:17px;">
                ...
                ">
                  ...
                  <span style="
                    padding:5px 9px;
                    border-radius:20px;
                    background:rgba(164,217,111,.10);
                    color:#a4d96f;
                    font-size:10px;
                    font-weight:700;
                  ">
                ...
                ">
                  📦 Stock disponible:
                  <strong style="color:var(--text-strong,#eef2f4);">
                    ${p.stock}
                  </strong>
                </div>
      ...
      ">
        ...
        ">
          ...
          <p style="
            ...
            font-size:12px;
          ">
            Productos vendidos, compradores, precios y fechas.
          </p>
